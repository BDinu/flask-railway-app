...
def add_manual_measurement():
    ...
    return jsonify({'message': 'Manual measurement added successfully'}), 201
...
def delete_measurement(measurement_id):
    ...
    if affected == 0:
        return jsonify({'error': 'Measurement not found or unauthorized'}), 404
...
def export_measurements():
    ...
    if not measurements:
        return jsonify({'error': 'No measurements available to export'}), 404
...
def save_all_current_measurements():
    ...
    if not locations or not isinstance(locations, list):
        return jsonify({'error': 'Locations list required'}), 400
    ...
    if saved == 0:
        return jsonify({'error': 'No valid measurements saved'}), 400
    return jsonify({'message': f'{saved} measurements saved successfully'}), 201
...
def calculate_aqi(pm25=None, pm10=None, co=None, no2=None, so2=None, o3=None):
    ...
    if so2 is not None: # SO2 (ppb) - OpenWeather provides in μg/m3, conversion needed
        # Assuming typical atmospheric conditions for conversion (approx 1 ppb SO2 = 2.62 μg/m3)
    ...
    if o3 is not None: # O3 (ppb) - OpenWeather provides in μg/m3, conversion needed
        # Assuming typical atmospheric conditions for conversion (approx 1 ppb O3 = 1.96 μg/m3)
...
def plot_all_graphs():
    ...
    if not data:
        return jsonify({'error': 'No measurements found to plot'}), 404
    ...
    plots = {}
    if temps:
        plots['temperature_plot'] = create_plot(dates, temps, 'Temperature Over Time', 'Timestamp', 'Temperature (°C)')
    if hums:
        plots['humidity_plot'] = create_plot(dates, hums, 'Humidity Over Time', 'Timestamp', 'Humidity (%)')
    if pm25s:
        plots['pm25_plot'] = create_plot(dates, pm25s, 'PM2.5 Over Time', 'Timestamp', 'PM2.5 (µg/m3)')
    if pressures:
        plots['pressure_plot'] = create_plot(dates, pressures, 'Pressure Over Time', 'Timestamp', 'Pressure (hPa)')
    if cos:
        plots['co_plot'] = create_plot(dates, cos, 'CO Over Time', 'Timestamp', 'CO (µg/m3)')
    if no2s:
        plots['no2_plot'] = create_plot(dates, no2s, 'NO2 Over Time', 'Timestamp', 'NO2 (µg/m3)')
    if so2s:
        plots['so2_plot'] = create_plot(dates, so2s, 'SO2 Over Time', 'Timestamp', 'SO2 (µg/m3)')
    if o3s:
        plots['o3_plot'] = create_plot(dates, o3s, 'O3 Over Time', 'Timestamp', 'O3 (µg/m3)')
    if aqi_values:
        plots['aqi_plot'] = create_plot(dates, aqi_values, 'Simplified AQI Over Time', 'Timestamp', 'AQI')

    # Correlation plots
    if len(temps) > 1 and len(hums) > 1: # Need at least 2 points for correlation
        plots['temp_vs_humidity_plot'] = create_plot(temps, hums, 'Temperature vs. Humidity', 'Temperature (°C)', 'Humidity (%)', plot_type='scatter')
    if len(temps) > 1 and len(pressures) > 1:
        plots['temp_vs_pressure_plot'] = create_plot(temps, pressures, 'Temperature vs. Pressure', 'Temperature (°C)', 'Pressure (hPa)', plot_type='scatter')
    ...
    for i in range(len(pollutant_names)):
        for j in range(i + 1, len(pollutant_names)):
            ...
            # Only plot if both lists have data and same length (for direct correlation)
            # Or handle mismatched lengths (e.g., interpolate, or only use common timestamps)
            # For simplicity, we'll assume they align if present in `data` from DB.
            # A more robust solution would ensure timestamps match or filter accordingly.
